...
export abstract class BaseCrudService<T extends ObjectLiteral> {
  ...
  async create(payload: DeepPartial<T>, manager?: EntityManager) {
    const repo = manager
  ...
  async update(id: string, payload: DeepPartial<T>, manager?: EntityManager) {
    const repo = manager
      ? manager.getRepository(this.repository.target)
      : this.repository;
  ...
  async findOneByIdCached(id: string, ttl = 60): Promise<T | null> {
